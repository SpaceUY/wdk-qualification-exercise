...
- Only reachable when the self-hosted stack's `app-node` is running and reachable (directly by the client for registration, and by the backend server-side via `WDK_APP_NODE_BASE_URL` for the proxy) — the stack is deployed and live at `https://cashback-wdk-stack.spacedev.io` (see [`infra/wdk-stack`](../../infra/wdk-stack/README.md)), not local-only
...
| `EXPO_PUBLIC_APP_NODE_URL` | `http://localhost:3000` (deployed: `https://cashback-wdk-stack.spacedev.io`) | Base URL of the self-hosted WDK stack's `app-node` (see [`infra/wdk-stack`](../../infra/wdk-stack/README.md)), used directly for wallet registration only — transaction history is proxied through the backend instead (see Transaction history above) |
