...
# Chain-agnostic image — reused for every (chain,token) processor instance in doc 02 by varying
# CHAIN/TOKEN/RACK env vars at compose level, not by rebuilding.
#
...
COPY --from=entrypoints processor.entrypoint.js ./docker-entrypoint.js
...
USER 65532:65532
EXPOSE 8080
LABEL org.opencontainers.image.title="wdk-indexer-processor-wrk" \
      org.opencontainers.image.description="WDK Transaction Router — one instance per (chain,token) pair" \
      org.opencontainers.image.source="https://github.com/SpaceUY/wdk-indexer-processor-wrk" \
