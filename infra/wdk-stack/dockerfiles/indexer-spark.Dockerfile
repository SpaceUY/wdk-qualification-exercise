...
#
# wdk-indexer-wrk-spark does NOT ship a production Dockerfile upstream (only a non-production
# CI.Dockerfile) — this one is new, modeled on wdk-indexer-wrk-btc's (the only fork that ships
# a real one), so config generation goes through the shared entrypoint-lib pattern (see
# docs/wdk-self-hosted-stack/01-core-pipeline.md §3) instead of a repo-native entrypoint.
#
...
COPY --from=entrypoints indexer-spark.entrypoint.js ./docker-entrypoint.js
...
LABEL org.opencontainers.image.title="wdk-indexer-wrk-spark" \
      org.opencontainers.image.description="WDK Spark (Lightning L2) indexer worker (proc + api)" \
      org.opencontainers.image.source="https://github.com/SpaceUY/wdk-indexer-wrk-spark" \
