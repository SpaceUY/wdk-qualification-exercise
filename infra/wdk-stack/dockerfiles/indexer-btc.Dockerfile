...
#
# wdk-indexer-wrk-btc ships its own production Dockerfile upstream, but we replace it with our
# own so config generation goes through the shared entrypoint-lib pattern (see
# docs/wdk-self-hosted-stack/01-core-pipeline.md §3) instead of the repo's own entrypoint.
#
...
COPY --from=entrypoints indexer-btc.entrypoint.js ./docker-entrypoint.js
...
LABEL org.opencontainers.image.title="wdk-indexer-wrk-btc" \
      org.opencontainers.image.description="WDK Bitcoin indexer worker (proc + api)" \
      org.opencontainers.image.source="https://github.com/SpaceUY/wdk-indexer-wrk-btc" \
